    ...
    try:
        ...
        cursor = await db.execute(f"""
            SELECT
                ...
                COALESCE(SUM(CASE WHEN transaction_type='buy' THEN total_value ELSE 0 END), 0) as total_buy,
                COALESCE(SUM(CASE WHEN transaction_type='sell' THEN total_value ELSE 0 END), 0) as total_sell,
                COUNT(DISTINCT ticker) as unique_tickers,
                (SELECT sector FROM transactions t2 WHERE t2.filer_name = transactions.filer_name
