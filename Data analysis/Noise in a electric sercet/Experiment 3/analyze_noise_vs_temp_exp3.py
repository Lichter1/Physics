#!/usr/bin/env python3
"""
Script to analyze noise amplitude vs temperature relationship.
Experiment 3: Fixed R = 1 kΩ, Δf = 250 Hz, varying temperature (T)

From Johnson-Nyquist: V² = 4kTRΔf
With fixed R and Δf, we expect: V² = (4kRΔf) · T = slope · T
So noise power should scale linearly with temperature.

This script:
- Converts dB values to V²
- Applies preamplifier and window corrections
- Fits a linear function V² = a·T + b
- Derives physical quantities from the fit (Boltzmann constant)
"""
...
TEMPERATURE_ERR = 0.3  # K (uncertainty in temperature, from folder name)

# Resistance: R_total = R_resistor + R_0 (internal resistance from Experiment 1)
R_RESISTOR = 1000  # Ω (1 kΩ nominal resistor)
R_RESISTOR_ERR = 10  # Ω (resistor tolerance ~1%)
...
RESISTANCE = R_RESISTOR + R_0  # 1419 Ω
RESISTANCE_ERR = np.sqrt(R_RESISTOR_ERR**2 + R_0_ERR**2)  # ~281 Ω

# Bandwidth (fixed for this experiment)
BANDWIDTH = 250  # Hz
BANDWIDTH_ERR = 0  # Hz (assuming negligible uncertainty)
...
def parse_results_file(filepath):
    """
    Parse the noise analysis results file for Experiment 3.

    Format:
    Temperature | μ (Linear) | σ (Linear) | Mean (dB)
    ...
    for line in lines:
        if '°C' in line and '|' in line and 'Temperature' not in line:
            parts = line.split('|')

            # Extract temperature (remove °C)
            temp_str = parts[0].strip().replace('°C', '').strip()
    ...
    return np.array(temperatures), np.array(mu_linear), np.array(sigma_linear), np.array(mean_db)
...
def calculate_boltzmann_constant(a, a_err, R, R_err, delta_f, delta_f_err):
    """
    Calculate Boltzmann constant from slope of V² vs T fit.

    From Johnson-Nyquist relation: V² = 4kTRΔf
    Slope a = 4kRΔf, so k = a / (4RΔf)

    Parameters:
    -----------
    a : float
        Slope from linear fit (V²/K)
    a_err : float
    ...
    """
    k = a / (4 * R * delta_f)
    # Error propagation: (Δk/k)² = (Δa/a)² + (ΔR/R)² + (ΔΔf/Δf)²
    relative_err_a = a_err / a
    relative_err_R = R_err / R if R_err > 0 else 0
    relative_err_df = delta_f_err / delta_f if delta_f_err > 0 else 0
    relative_err_k = np.sqrt(relative_err_a**2 + relative_err_R**2 + relative_err_df**2)
...
def main():
    ...
    results_file = script_dir / 'output/Experiment_3/results/noise_analysis_results.txt'

    # Create output directory
    output_dir = script_dir / 'output' / 'Experiment_3' / 'results'
    ...
    print("="*60)
    print("EXPERIMENT 3 - FIXED PARAMETERS")
    print("="*60)
    print(f"Bandwidth (Δf):   {BANDWIDTH} Hz")
    print(f"Resistance:")
    ...
    print(f"Temperature uncertainty: ± {TEMPERATURE_ERR} °C")
    print("Variable:         Temperature (T)")
    print("="*60 + "\n")

    # Check if results file exists
    if not results_file.exists():
        print(f"Error: Results file not found at {results_file}")
        print("Please run noise_filter_analysis_exp3.py first to generate the results.")
        return

    # Parse data (now in linear power scale from Gaussian fit)
    print("Parsing data from results file...")
    temperatures_celsius, v2_linear, v2_sigma_linear, v2_db_ref = parse_results_file(results_file)

    # Convert temperatures to Kelvin
    temperatures_kelvin = temperatures_celsius + 273.15

    print(f"\nData from Gaussian fit (Linear Power Scale):")
    print(f"  Temperatures (°C): {temperatures_celsius}")
    print(f"  Temperatures (K):  {temperatures_kelvin}")
    ...
    # Perform weighted linear fit: V² = a*T + b (T in Kelvin)
    print("\nPerforming weighted linear regression...")
    a, b, a_err, b_err, chi2, reduced_chi2 = \
        perform_weighted_linear_fit(temperatures_kelvin, v2_corr, v2_sigma_corr)
    ...
    # Display fit results
    print("\n" + "="*60)
    print(f"LINEAR FIT RESULTS: V² = a·T + b ({SIGMA_MULTIPLIER}σ uncertainties)")
    print("="*60)
    print(f"Slope (a):      {a:.4e} ± {a_err_reported:.4e} V²/K")
    print(f"Intercept (b):  {b:.4e} ± {b_err_reported:.4e} V²")
    print(f"χ²:             {chi2:.4f}")
    print(f"χ²/dof:         {reduced_chi2:.4f}")
    print(f"Degrees of freedom: {len(temperatures_kelvin) - 2}")
    print("="*60)
    ...
    k_measured, k_err_1sigma = calculate_boltzmann_constant(a, a_err, RESISTANCE, RESISTANCE_ERR, BANDWIDTH, BANDWIDTH_ERR)
    k_err_reported = k_err_1sigma * SIGMA_MULTIPLIER

    print(f"\nBoltzmann constant (k):")
    print(f"  From: V² = 4kTRΔf, slope a = 4kRΔf")
    print(f"  k = a / (4RΔf)")
    ...
    print(f"    From bandwidth (ΔΔf/Δf): {100*BANDWIDTH_ERR/BANDWIDTH:.2f}%" if BANDWIDTH_ERR > 0 else "    From bandwidth (ΔΔf/Δf): 0.00%")
    ...
    expected_slope = 4 * BOLTZMANN_EXPECTED * RESISTANCE * BANDWIDTH
    print(f"\nExpected slope (from known k):")
    print(f"  a_expected = 4kRΔf = {expected_slope:.4e} V²/K")
    print(f"  a_measured = {a:.4e} V²/K")
    ...
    ax.errorbar(temperatures_kelvin, v2_corr, yerr=v2_sigma_corr * SIGMA_MULTIPLIER,
                fmt='o', markersize=8, capsize=5, capthick=2,
                label=f'Measured data ({SIGMA_MULTIPLIER}σ error bars)', color='blue', ecolor='blue', alpha=0.7)

    # Plot fit line
    x_fit = np.linspace(min(temperatures_kelvin) * 0.9, max(temperatures_kelvin) * 1.1, 100)
    y_fit = a * x_fit + b
    ax.plot(x_fit, y_fit, 'r-', linewidth=2,
            label=f'Linear fit: V² = a·T + b')
    ...
    ax.set_xlabel('Temperature T (K)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Noise Amplitude V²_corr (V²)', fontsize=12, fontweight='bold')
    ax.set_title(f'White Noise Amplitude vs Temperature - Experiment 3\n(R={RESISTANCE/1000:.2f}kΩ, Δf={BANDWIDTH}Hz)',
