#!/usr/bin/env python3
"""
White Noise Filtering and Statistical Analysis - Experiment 2
...
def analyze_single_bandwidth(filepath, output_dir=".", show_plots=True):
    """
    Analyze a single bandwidth measurement file.
    ...
    filtered_dir = output_dir / "output" / "Experiment_2" / "filtered_plots"
    distribution_dir = output_dir / "output" / "Experiment_2" / "distribution_plots"
    ...
    print(f"\n{'='*70}")
    print(f"Analyzing: {filepath.name} (Bandwidth = {bw_label})")
    print(f"{'='*70}")
    ...
    ax.set_title(f'White Noise Filtering - Bandwidth = {bw_label} (R=68.3kΩ, T=14.6°C)',
    ...
    output_path = filtered_dir / f"filtered_{bw_filename}hz.png"
    ...
    ax_dist.set_title(f'Distribution of White Noise Power - Bandwidth = {bw_label}',
    ...
    dist_output_path = distribution_dir / f"distribution_{bw_filename}hz.png"
...
def analyze_all_bandwidths(data_folder="experiment 2 - tempreture 14.6 R=68.3kohm",
                           output_dir=".", show_plots=False):
    """Analyze all bandwidth measurements in a folder."""

    data_folder = Path(data_folder)
    results = {}

    print("\n" + "="*70)
    print("ANALYZING ALL BANDWIDTH MEASUREMENTS")
    print("="*70)
    ...
    for data_file in data_files:
        bandwidth = parse_bandwidth(data_file.name)
        if bandwidth is not None:
            result = analyze_single_bandwidth(data_file, output_dir=output_dir,
                                             show_plots=show_plots)
            results[bandwidth] = result

    # Create summary table
    print("\n" + "="*85)
    print("SUMMARY TABLE - ALL BANDWIDTHS (Linear Power Scale)")
    print("="*85)
    print(f"{'Bandwidth':>12} | {'μ (Linear)':>14} | {'σ (Linear)':>14} | {'Mean (dB)':>12} | {'Points':>7}")
    print("-" * 85)

    for bandwidth in sorted(results.keys()):
        r = results[bandwidth]
        bw_label = f"{bandwidth:.2f} Hz" if bandwidth < 10 else f"{bandwidth:.1f} Hz"
        print(f"{bw_label:>12} | {r['mu_linear']:>14.6e} | {r['sigma_linear']:>14.6e} | {r['mean_db']:>12.4f} | {len(r['frequencies']):>7}")
...
def main():
    ...
    if True:  # Analyze all bandwidths
        # Analyze all bandwidths
        results = analyze_all_bandwidths(
        ...
        results_dir = script_dir / "output" / "Experiment_2" / "results"
        results_dir.mkdir(parents=True, exist_ok=True)
        results_file = results_dir / "noise_analysis_results.txt"
        with open(results_file, "w") as f:
            f.write("White Noise Analysis Results - Experiment 2 (Linear Power Scale)\n")
            f.write("Fixed: R = 68.3 kΩ, T = 14.6°C\n")
            f.write("Variable: FFT Bandwidth\n")
            f.write("="*90 + "\n\n")
            f.write("Gaussian fit performed on LINEAR power values (correct for thermal noise analysis)\n")
            f.write("Power = 10^(dB/10)\n\n")
            f.write(f"{'Bandwidth':>12} | {'μ (Linear)':>14} | {'σ (Linear)':>14} | {'Mean (dB)':>12}\n")
            f.write("-" * 90 + "\n")
            for bandwidth in sorted(results.keys()):
                r = results[bandwidth]
                bw_label = f"{bandwidth:.2f} Hz" if bandwidth < 10 else f"{bandwidth:.1f} Hz"
                f.write(f"{bw_label:>12} | {r['mu_linear']:>14.6e} | {r['sigma_linear']:>14.6e} | {r['mean_db']:>12.4f}\n")

        print(f"\nResults saved to: {results_file}")

    else:
        # Test with single bandwidth file only
        data_file = data_folder / "125hz.txt"

        if data_file.exists():
            result = analyze_single_bandwidth(data_file, output_dir=script_dir, show_plots=True)
